...
const App: React.FC = () => {
    ...
    const initializeAuth = async () => {
      try {
        ...
        if (session?.user) {
          ...
          // Verificar se o perfil atual está bloqueado (caso esteja logado)
          if (newUserState.currentProfileId) {
            const currentP = profiles.find(p => p.id === newUserState.currentProfileId);
            if (currentP?.is_blocked) {
              alert("🚫 Este perfil foi bloqueado pelo administrador.");
              newUserState.currentProfileId = null;
              setScreen('PICKER');
            }
          }
        } else {
  ...
  const handleProfileSelect = (profileId: string) => {
    ...
    if (profile?.is_blocked) {
      alert("🚫 Este perfil está bloqueado e não pode acessar o sistema.");
      return;
    }
  ...
  const handleProfileUpdate = async (id: string, name: string, avatar: string, bio?: string) => {
  ...
  const handleBuyItem = async (item: typeof SHOP_ITEMS[0]) => {
    ...
    const newCoins = currentProfile.coins - item.price;
    const newUnlockedItems = [...currentProfile.unlockedItems, item.id];
    ...
    try {
      ...
      // Em caso de erro, idealmente reverteríamos o estado
    }
  };
  ...
  const renderScreen = () => {
    ...
    switch (screen) {
              ...
              <div className="relative z-10">
                ...
                <div className={`grid gap-8 ${isAdult ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'}`}>
                  <div
                    ...
                    className={`group relative overflow-hidden rounded-[3rem] p-10 border-4 transition-all cursor-pointer ${isVerseDone
                  ...
                  {!isAdult && (
                    <div
                      ...
                      className={`group relative overflow-hidden rounded-[3rem] p-10 border-4 transition-all cursor-pointer ${isArtDone
                        ? 'bg-emerald-50 border-emerald-200 dark:bg-emerald-900/20 dark:border-emerald-800'
      ...
      case 'SHOP': return <Shop coins={currentProfile.coins || 0} unlockedItems={currentProfile.unlockedItems || []} onBuy={handleBuyItem} onBack={() => setScreen('HOME')} />;
      case 'ART_MISSION': return <PaintingRoom onSave={handleArtChallengeComplete} savedPaintings={currentProfile.gallery || []} unlockedItems={currentProfile.unlockedItems || []} onNavigateToShop={() => setScreen('SHOP')} initialMode={paintingMode} artMissionTheme={currentProfile.artMissionTheme} onArtMissionThemeGenerated={handleArtMissionThemeGenerated} isArtMissionCompleted={isArtDone} onBack={() => setScreen('HOME')} />;
      case 'BIBLE': return <BibleReader onFavorite={toggleFavorite} favorites={currentProfile.favorites} onBack={() => setScreen('HOME')} />;
      case 'CHALLENGES': return <VerseChallenges onComplete={handleVerseChallengeComplete} onGoToBible={() => setScreen('BIBLE')} lastChallengeDate={currentProfile.lastChallengeDate} unlockedItems={currentProfile.unlockedItems || []} onBack={() => setScreen('HOME')} />;
      case 'READING': return <BibleReadingKids onSaveRecording={handleSaveRecording} recordings={currentProfile.recordings || []} onBack={() => setScreen('HOME')} />;
      case 'GALLERY': return <GalleryView images={currentProfile.gallery || []} onBack={() => setScreen('HOME')} />;
      case 'PROFILE': return <ProfileView profile={currentProfile} onUpdateProfile={handleProfileUpdate} onSwitchProfile={() => setScreen('PICKER')} onLogout={logout} onBack={() => setScreen('HOME')} />;
  ...
  return (
    <div className={`min-h-screen transition-all duration-300 dark:bg-gray-900`}>
      {user.isAuthenticated && screen !== 'PICKER' && screen !== 'AUTH' && currentProfile && (
        <Navigation currentScreen={screen} profileType={currentProfile.type} unlockedItems={currentProfile.unlockedItems} onNavigate={(s) => setScreen(s)} onLogout={logout} theme={user.theme} onToggleTheme={toggleTheme} isAdmin={currentProfile.is_admin} />
      )}
