...
const Shop: React.FC<ShopProps> = ({ coins, unlockedItems, onBuy }) => {
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto animate-in fade-in duration-700">
      {/* Header com Saldo */}
      <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-6 bg-white dark:bg-gray-800 p-8 rounded-[3rem] shadow-xl border-4 border-indigo-50 dark:border-gray-700">
      ...
      <div className="flex overflow-x-auto pb-4 gap-3 no-scrollbar mb-8 mask-fade-right">
        {CATEGORIES.map((cat) => (
          <button
            ...
            className={`flex items-center space-x-3 px-8 py-4 rounded-2xl font-black whitespace-nowrap transition-all shadow-md active:scale-95 ${
              activeTab === cat.id 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-12">
        {filteredItems.map((item) => {
          ...
          return (
            <div 
              key={item.id}
              className={`group bg-white dark:bg-gray-800 rounded-[3rem] p-6 shadow-xl border-4 transition-all flex flex-col items-center text-center relative overflow-hidden ${
                isCelebrating ? 'animate-bounce border-yellow-400 z-50' : 
            ...
            >
              ...
              <button
                ...
                className={`w-full py-5 rounded-[2rem] font-black text-lg shadow-lg transition-all flex items-center justify-center space-x-3 relative z-10 ${
                  isUnlocked 
