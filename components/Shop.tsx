...
const Shop: React.FC<ShopProps> = ({ coins, unlockedItems, onBuy, onBack }) => {
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto animate-in fade-in duration-700 font-outfit relative">
      ...
      <div className="relative z-10">
        {/* Header com Saldo */}
        <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-6 bg-white/70 dark:bg-gray-900/60 backdrop-blur-xl p-8 rounded-[3rem] shadow-xl border border-white/50 dark:border-gray-800/50 relative transition-all duration-300">
          <button
            onClick={onBack}
          ...
          <div className="text-center md:text-left md:pl-20">
            <h2 className="text-4xl md:text-5xl font-black text-indigo-900 dark:text-white tracking-tight">Lojinha do FiliF</h2>
        ...
        <div className="flex overflow-x-auto pb-4 gap-3 no-scrollbar mb-8 mask-fade-right">
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`flex items-center space-x-3 px-8 py-4 rounded-2xl font-black whitespace-nowrap transition-all shadow-md active:scale-95 border ${activeTab === cat.id
                ? 'bg-indigo-600 text-white scale-105 border-indigo-600'
                : cat.status === 'COMING_SOON'
                  ? 'bg-gray-100 dark:bg-gray-900 text-gray-300 dark:text-gray-700 cursor-not-allowed border-dashed border-gray-200 dark:border-gray-800'
                  : 'bg-white/80 dark:bg-gray-800/80 backdrop-blur-md text-gray-500 dark:text-gray-400 border-white/20 dark:border-gray-700 hover:bg-indigo-50 dark:hover:bg-gray-800'
                }`}
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-12">
          {filteredItems.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`group bg-white/70 dark:bg-gray-900/60 backdrop-blur-md rounded-[3rem] p-6 shadow-xl border-4 transition-all flex flex-col items-center text-center relative overflow-hidden duration-300 ${isCelebrating ? 'animate-bounce border-yellow-400 z-50' :
                  isUnlocked ? 'border-emerald-100 dark:border-emerald-900/40 opacity-90' : 'border-white/50 dark:border-gray-800/50 hover:border-indigo-200 dark:hover:border-indigo-900 hover:-translate-y-2 hover:shadow-2xl'
                  }`}
              >
                ...
                <div className="w-24 h-24 rounded-[2rem] bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 flex items-center justify-center text-6xl mb-6 shadow-inner transform group-hover:rotate-6 transition-transform relative z-10 border border-white/20 dark:border-gray-700">
                  {item.icon}
                ...
                <button
                  ...
                  className={`w-full py-5 rounded-[2rem] font-black text-lg shadow-lg transition-all flex items-center justify-center space-x-3 relative z-10 ${isUnlocked
                    ...
                    : canAfford
                      ? 'bg-indigo-600 text-white hover:scale-105 active:scale-95 hover:bg-indigo-700 shadow-indigo-500/30'
                ...
                {!isUnlocked && !canAfford && (
                  <div className="mt-4 flex items-center justify-center space-x-1 text-red-400 bg-red-50 dark:bg-red-900/20 px-4 py-2 rounded-full">
        ...
        {/* Dicas de Ganho */}
        <div className="bg-indigo-50/50 dark:bg-indigo-900/10 backdrop-blur-sm p-10 rounded-[4rem] border-4 border-dashed border-indigo-100 dark:border-indigo-800 relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-[2.5rem] shadow-sm flex flex-col items-center text-center hover:scale-105 transition-transform">
