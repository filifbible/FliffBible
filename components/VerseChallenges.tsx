...
const VerseChallenges: React.FC<VerseChallengesProps> = ({ onComplete, onGoToBible, lastChallengeDate, unlockedItems }) => {
  ...
  if (step === 'SUCCESS') {
    return (
      <div className="p-8 text-center max-w-2xl mx-auto mt-20 bg-white dark:bg-gray-800 rounded-[3rem] shadow-xl border-4 border-emerald-100 dark:border-emerald-900 animate-in zoom-in-95">
        ...
        <div className="bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 font-black p-6 rounded-2xl border-2 border-emerald-100 dark:border-emerald-800">
           MOEDA COLETADA ✅
        </div>
      </div>
    );
          ...
          <div className="space-y-3 mb-10">
            {challenge?.options.map((opt, idx) => (
                <button
                  ...
                  className={`w-full p-5 rounded-2xl font-bold transition-all border-4 relative overflow-hidden ${
                    selectedOption === idx 
