...
const PaintingRoom: React.FC<PaintingRoomProps> = ({
  ...
  if (mode === 'PHYSICAL') {
    return <PhysicalArtMission onSave={(img) => onSave(img, true)} onCancel={() => setMode('SELECTION')} savedTheme={artMissionTheme} onThemeGenerated={onArtMissionThemeGenerated} isCompleted={isArtMissionCompleted} onHome={onBack} />;
  }
